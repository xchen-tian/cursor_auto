...
Watch 和 Scan 模式同时内置 **Claude Code 扩展自动审批**：当 Claude Code 弹出权限确认对话框（如 "Allow this bash command?"）时，自动点击 Yes。该功能与 Composer 点击共享同一循环，暂停/继续完全同步，不会抢占焦点。
...
**Q: Claude Code 扩展的权限对话框也能自动审批吗？**
A: 是的，Watch 和 Scan 模式已内置 Claude Code 自动审批。每轮循环会扫描所有 Claude Code webview target，发现权限对话框自动点击 Yes。指示器会短暂显示 `CC: APPROVE`。
